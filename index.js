...
const MATCH_WINDOW_MS = 5 * 60 * 1000;  // 5 минут — времязона на сайте может отличаться от телефона
...
// ВСЕГДА перезаписываем — env-переменная имеет приоритет над случайно закоммиченным файлом.
...
async function ensureLoggedIn(page) {
  ...
  console.warn('not logged in. First-time setup: run with HEADLESS=false, ' +
               'enter token + 2FA manually, bot will save storage_state.json.');
  return false;
}
...
async function findMatchingRows(page, amount) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (rowAmount === amount) {
      ...
      matches.push({ index: i, row, rowTimeMs, localText, timeText });
    }
  }
  return matches;
}
...
async function main() {
  ...
  if (!MOCK_SITE) {
    ...
    if (!logged && HEADLESS) {
      console.error('FATAL: not logged in and running headless. Run once with HEADLESS=false to capture session.');
      process.exit(1);
    }
    if (!logged) {
      console.log('Waiting up to 5 min for manual login (token + 2FA)…');
      try {
        await page.locator(SELECTORS.loginSuccessIndicator).first().waitFor({ timeout: 5 * 60_000 });
        console.log('Manual login captured.');
      } catch {
