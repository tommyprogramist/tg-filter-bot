...
async function performLogin(page) {
  ...
  try {
    ...
    // Submit — кнопка ВНУТРИ той же формы что token-input. Иначе можем тапнуть
    // по "Login" формы trader'а.
    const tokenForm = tokenInput.locator('xpath=ancestor::form[1]');
    const submitBtn = (await tokenForm.count() > 0)
      ? tokenForm.locator(SELECTORS.loginTokenSubmit).first()
      : page.locator(SELECTORS.loginTokenSubmit).last();  // fallback: последняя кнопка
